...
fun main(args: Array<String>) {
    ...
    engineSchematicLines.forEachIndexed { index, engineSchematicLine ->
        if (engineSchematicLine.lineNumber > 0 && engineSchematicLine.lineNumber < engineSchematicLines.size - 1) {
            val partSum = engineSchematicLine.getEnginePartNumbersSum(
...
public class EngineSchematicLine(val lineStr: String, val lineNumber: Int) {
    ...
    private fun getEnginePartNumbers(
        previousEngineSchematicLine: EngineSchematicLine,
        nextEngineSchematicLine: EngineSchematicLine
    ): List<LineInteger> {
        return lineIntegers?.filter {
            val prevHas =
                previousEngineSchematicLine.hasSymbolByPosition(it.position.start - 1..it.position.endInclusive + 1)
            val nextHas =
                nextEngineSchematicLine.hasSymbolByPosition(it.position.start - 1..it.position.endInclusive + 1)
            val currentHas = hasSymbolByPosition(it.position.start - 1..it.position.endInclusive + 1)
            currentHas || prevHas || nextHas
        }?.toList() ?: listOf()
    }

    public fun getEnginePartNumbersSum(
        previousEngineSchematicLine: EngineSchematicLine,
        nextEngineSchematicLine: EngineSchematicLine
    ): Int {
        lineIntegers = findNumbersWithPositions(lineStr);
        var lineSum = 0;
        getEnginePartNumbers(previousEngineSchematicLine, nextEngineSchematicLine).forEach {
            lineSum += it.value
        }
        return lineSum
    }
