...
object Pipes {
    ...
    fun pipeAt(x: Int, y: Int): Char = pipes[x][y]
    ...
    val rD = Point(1, 0)  //right direction
    val dD = Point(0, 1)  //down direction
    val lD = Point(-1, 0)  //left direction

    var currentPos: Point
    var direction: Point
    var NO_DIRECION = Point(0,0)
    init {
    ...
    fun getNextDirection() = when (pipeAt(currentPos)) {
        '|' -> Point(0, direction.y)
        '-' -> Point(direction.x, 0)
        'L' -> Point(direction.x + 1, direction.y - 1)
        'J' -> Point(direction.x - 1, direction.y - 1)
        '7' -> Point(direction.x - 1, direction.y + 1)
        'F' -> Point(direction.x + 1, direction.y + 1)
        'S' -> NO_DIRECION
