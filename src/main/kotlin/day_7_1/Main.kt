...
fun main1() {
    while(true) {
        ...
        println("${firstHand.hand} $compareChar ${secondHand.hand}")
    }
}
...
class Hand(val hand: String, val bid: Int) : Comparable<Hand> {
    ...
    fun getHandValue(): Long {
        val placeValue = 13.0
        val placeValues = mapOf( 'A' to 13, 'K' to 12, 'Q' to 11, 'J' to 10, 'T' to 9, '9' to 8, '8' to 7, '7' to 6, '6' to 5, '5' to 4, '4' to 3, '3' to 2, '2' to 1)
        val sameCards = mutableMapOf<Char, Int>()
        var value = 0L
        for ((index, c) in hand.withIndex()) {
            value += placeValues[c]!! * 13.0.pow(4 - index).toLong()
        ...
        when {
            isFiveOfAKind(sameCards) -> value += placeValue.toDouble().pow(10).toLong()
            isFourOfAKind(sameCards) -> value += placeValue.toDouble().pow(9).toLong()
            isFullHouse(sameCards) -> value += placeValue.toDouble().pow(8).toLong()
            threeOfAKind(sameCards) -> value += placeValue.toDouble().pow(7).toLong()
            twoPair(sameCards) -> value += placeValue.toDouble().pow(6).toLong()
            onePair(sameCards) -> value += placeValue.toDouble().pow(5).toLong()
        }
        return value
    }
    ...
    fun threeOfAKind(sameCards: Map<Char, Int>) : Boolean {
        if (sameCards.size!=3) {
            return false
        }
        return sameCards.keys.filter {
    ...
    fun twoPair(sameCards: Map<Char, Int>) : Boolean {
        if (sameCards.size!=3) {
            return false
        }
        return !sameCards.keys.filter {
    ...
    fun onePair(sameCards: Map<Char, Int>) = sameCards.size==4
...
var x = "A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, 2"
