...
fun main(args: Array<String>) {
    ...
    val engineSchematicLines = mutableListOf<EngineSchematicLine>();
    lineList.forEachIndexed { index, value ->
        engineSchematicLines.add(EngineSchematicLine(value, index))
    }
    var sum = 0;
    engineSchematicLines.forEachIndexed { index, engineSchematicLine ->
        if (engineSchematicLine.lineNumber > 0 && engineSchematicLine.lineNumber < engineSchematicLines.size - 1) {
            val partSum = engineSchematicLine.getGearPartNumbersSum(
                engineSchematicLines[index - 1],
                engineSchematicLines[index + 1]
            )
            println(partSum)
            sum += partSum
...
class LineInteger(val value: Int, val position: IntRange) {
    override fun toString() = "LineInteger(value=$value, position=$position)"
    public fun isOverlapped(range: IntRange): Boolean {
        return (position.endInclusive >= range.start) && (position.start <= range.endInclusive)
    }
}

class LineAsterisk(val position: IntRange) {
    var multiple:Int = 0
    override fun toString() = "LineAsterisk(position=$position)"
}


// ------------------------------------------------------------------------------------------------------------

public class EngineSchematicLine(val lineStr: String, val lineNumber: Int) {

    companion object {
        var adjacentCount: Int = 2
    }

    private var lineIntegers: List<LineInteger>? = null
    private var lineAsterisks: List<LineAsterisk>? = null

    init {
        this.lineIntegers = findNumbersWithPositions(lineStr);
    }

    public fun getIntegerSiblings(range: IntRange): List<Int> {
        return lineIntegers?.filter { it.isOverlapped(range) }?.map { it.value }?.toList() ?: listOf()
    }

    fun hasSymbolByPosition(pos: IntRange): Boolean {
        var positionStr = lineStr.substring(pos)
        //println(positionStr)
        val pattern = Regex("[^0-9.]") // TODO OBJECT legyen
        return pattern.containsMatchIn(positionStr)
    }

    private fun findNumbersWithPositions(input: String): List<LineInteger> {
        val pattern = Regex("\\b\\d+\\b")
        return pattern.findAll(input)
            .map { LineInteger(it.value.toInt(), it.range) }
            .toList()
    }

    private fun findAsterisksWithPositions(input: String): List<LineAsterisk> {
        val pattern = Regex("\\*")
        return pattern.findAll(input)
            .map { LineAsterisk(it.range) }
            .toList()
    }


    private fun setGearSiblingMultiple(
        previousEngineSchematicLine: EngineSchematicLine,
        nextEngineSchematicLine: EngineSchematicLine
    ) {
        lineAsterisks?.forEach {
            val prevLineSiblings =
                previousEngineSchematicLine.getIntegerSiblings(it.position.start - 1..it.position.endInclusive + 1)
            val nextLineSiblings =
                nextEngineSchematicLine.getIntegerSiblings(it.position.start - 1..it.position.endInclusive + 1)
            val currentLineSiblings = getIntegerSiblings(it.position.start - 1..it.position.endInclusive + 1)

            var multiple = 0;
            if (EngineSchematicLine.adjacentCount == prevLineSiblings.size + nextLineSiblings.size + currentLineSiblings.size) {
                (prevLineSiblings + nextLineSiblings + currentLineSiblings).forEach {
                    if (multiple == 0) {
                        multiple = it
                    } else {
                        multiple *= it
    ...
    public fun getGearPartNumbersSum(
        previousEngineSchematicLine: EngineSchematicLine,
        nextEngineSchematicLine: EngineSchematicLine
    ): Int {
        lineAsterisks = findAsterisksWithPositions(lineStr)
        setGearSiblingMultiple(previousEngineSchematicLine, nextEngineSchematicLine)
        var lineAsterisksSiblingIntegerSum = 0
        lineAsterisks!!.forEach {
            lineAsterisksSiblingIntegerSum += it.multiple
        }
        return lineAsterisksSiblingIntegerSum
    }
}
