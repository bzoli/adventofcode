...
fun main(args: Array<String>) {
    ...
    for (line in lineList) {
        setPower = 0;
        val sets = line.split(':')[1] // 8 green; 5 green, 6 blue, 1 red;
        val setMax : MutableMap<String, Int> = mutableMapOf("red" to 0, "green" to 0, "blue" to 0)
        for (set in sets.split(';')) { // 2 green, 1 blue, 4 red
            for (grab in set.split(',')) { // 2 green
                val colorSubset: List<String> = grab.split(" ")  //  , 2, green
                if (setMax[colorSubset[2]]!! < colorSubset[1].toInt()) {
                    setMax[colorSubset[2]] = colorSubset[1].toInt()
                }
            }
        }
        setPower = setMax["red"]!!  * setMax["green"]!! * setMax["blue"]!!
        print("${setMax["red"]}, ${setMax["green"]}, ${setMax["blue"]}: ")
        println(setPower)
        summa += setPower
        println(summa)
    }
...
var lines = """Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
